...
occur_collect <- 
  map2(bots, tops, 
       ~ occur_collect %>%
         ...
         mutate(bin_time = bin_ages(occur_mid_ma, by = 5, age = TRUE),
                bin_time = if_else(bin_time == max(bin_time), .y, bin_time))) %>%
  reduce(., rbind)
...
occur_count %>%
  ...
  scale_x_reverse(breaks = scales::pretty_breaks(n = 10)) +
  labs(x = 'Time (Mya)', y = 'Abundance', fill = 'Taxa')
...
occur_count %>%
  ...
  scale_fill_viridis(discrete = TRUE) +
  scale_x_reverse(breaks = scales::pretty_breaks(n = 10)) +
  labs(x = 'Time (Mya)', y = 'Relative Abundance', fill = 'Taxa')
...
Cave occurrences are the last occurrence of a taxon before a gap in their fossil record, excluding last occurrences. I'm calling these cave occurrences because of the story of Lazarus. Lazarus went into a cave, was gone for a while, and then reappeared. 

Something important to remember right now is that `bin_time` and `bin_num` *decrease* as we approach the present.


## Count number of Lazarus occurrences

The logic I'm using to identify Lazarus (and cave) occurrences is:

1. look at the occurrence record of each species,
2. identify each gap,
...
Here is all of that in action. And it is a mess.

```{r get_lazarus}

# first, let's just get genus occurrence over time
# i want to include the abundance of that genus in that bin just in case
...
genus_lazarus <- 
  ...
  filter(!(duplicated(genus))) %>%
  ungroup() %>%
  arrange(bin_num) %>%                 # temporal order (runs related)
...
lazarus_count %>%
  ...
  scale_fill_viridis(discrete = TRUE) +
  scale_x_reverse(breaks = scales::pretty_breaks(n = 10)) +
  labs(x = 'Time (Mya)', y = 'Lazarus occurrences', 
       fill = 'Taxa')

```

As expected there are multiple time intervals with large numbers of Lazarus occurrences, which we could consider a Lazarus effect. Interestingly, some of these intervals are not associated with the big 5 mass extinction events. 

Now let's also look at the cave occurrences. 
...
cave_count %>%
  ...
  scale_fill_viridis(discrete = TRUE) +
  scale_x_reverse(breaks = scales::pretty_breaks(n = 10)) +
  labs(x = 'Time (Mya)', y = 'Cave occurrences', 
       fill = 'Taxa')

```

The obvious pattern here is the the number of Lazarus and cave occurrences tends to increase as we approach the present. This makes sense because we're increaseing in both diversity and the quality of fossil record as we approach the present.
