...
occur_collect %>%
  ...
  count() %>%
  ggplot(aes(x = bin_time, y = n, colour = shelly)) +
  geom_line() +                        # geom_area doesn't play nice
...
genus_occur <- 
  ...
  group_by(genus) %>%
  mutate(lazarus = if_else(bin_num == max(bin_num), 0, lazarus)) %>%
  ungroup()

genus_occur %>%
  group_by(bin_time, shelly) %>%
  summarize(count = sum(lazarus)) %>%
  ggplot(aes(x = bin_time, y = count, colour = shelly)) +
  ...
  scale_colour_viridis(discrete = TRUE) +
  scale_x_reverse(breaks = scales::pretty_breaks(n = 10)) +
  labs(x = 'Time (Mya)', y = 'Relative Abundance', colour = 'Taxa')

```


There are multiple time intervals with an excess of Lazarus occurrences. Also, there are some intervals with a high number of Lazarus occurrences only for a single taxonomic group.
